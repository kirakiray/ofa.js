...
- [page.html](./examples/03-todolist/page.html): Todo List 页面模块，包含列表渲染和条件渲染。
- [demo.html](./examples/03-todolist/demo.html): 引用页面模块的入口文件。
...
- [demo.html](./examples/04-filelist/demo.html): 引用页面模块的入口文件。
- [page.html](./examples/04-filelist/page.html): 入口主页面。
- [filelist.html](./examples/04-filelist/filelist.html): 文件列表组件。
- [editor.html](./examples/04-filelist/editor.html): 文件内容查看组件。
- [data.js](./examples/04-filelist/data.js): 模拟数据和模拟获取数据的函数。

### 案例说明

1. 使用 `<o-provider name="providerName" bbb="value">` 标签包裹内容，可以让上下文内的组件使用 `<o-consumer name="providerName" watch:bbb="targetKey">` 标签监听对应`name`的 provider 属性变化。
2. 相比事件冒泡传递数据和属性方向传递，如果涉及很多层的数据传递，使用 o-provider 和 o-consumer 组件进行数据传递可以调高开发效率。
3. 子组件中使用 `this.getProvider(providerName)` 获取对应 `name` 的上下文对象，然后直接修改provider对象的属性即可同步到其他子组件。
4. 页面或组件模块，可以在 `watch` 中添加监听属性的函数，称为 watch 生命周期函数，只要对应key的属性变化，就会触发该函数。
5. `o-fill` 可以通过 `name` 渲染对应定义 `name` 的 template 的模板内容。而这个 template 模板可以递归渲染自身或其他 name 的 template。
