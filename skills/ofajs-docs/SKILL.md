...
- 简单的计数器页面：涵盖数据绑定、事件处理、计算属性和样式定义等基础用法。
...
- 待办事项列表：涵盖数据持久化、列表渲染、状态管理和本地存储等进阶用法。
  - [page.html](assets/03-todolist/page.html): 页面模块的定义文件，实现待办事项的增删改查和状态切换。
  - [data.js](assets/03-todolist/data.js): 数据管理模块，使用 `$.stanz` 创建响应式数据并持久化到 localStorage。
  - [demo.html](assets/03-todolist/demo.html): 引用页面模块的入口文件。
- 文件列表编辑器：涵盖嵌套组件通信、依赖注入和复杂组件交互等高级功能。
  - [page.html](assets/04-filelist/page.html): 页面模块的定义文件，使用 `o-provider` 实现组件间数据共享。
  - [filelist.html](assets/04-filelist/filelist.html): 文件列表组件，展示目录树和文件选择功能。
  - [editor.html](assets/04-filelist/editor.html): 文件编辑器组件，编辑选中文件的内容。
  - [data.js](assets/04-filelist/data.js): 模拟文件数据的模块。
  - [demo.html](assets/04-filelist/demo.html): 引用页面模块的入口文件。
- 单页应用路由：涵盖 o-router 路由库、o-app 应用配置和页面切换动画等路由功能。
  - [app-config.js](assets/05-routing/app-config.js): 应用配置文件，定义首页和页面切换动画。
  - [layout.html](assets/05-routing/layout.html): 布局页面，包含导航和页面容器。
  - [home.html](assets/05-routing/home.html): 首页内容。
  - [list.html](assets/05-routing/list.html): 列表页面，演示父子路由嵌套。
  - [detail.html](assets/05-routing/detail.html): 详情页面，演示动态路由参数。
  - [demo.html](assets/05-routing/demo.html): 引用路由配置的入口文件。
- 服务端渲染（SCSR）：涵盖 SEO 友好的服务端渲染方案和同构应用开发。
  - [home.html](assets/06-scsr/home.html): 首页，包含服务端渲染的模板结构。
  - [about.html](assets/06-scsr/about.html): 关于页面，演示 SCSR 页面切换。
  - [app-config.js](assets/06-scsr/app-config.js): 应用配置文件，定义页面切换动画。
  - [public.css](assets/06-scsr/public.css): 公共样式文件。
- API 操作示例：涵盖 Shadow DOM 操作、组件方法定义和外部调用等 API 用法。
  - [shadow-demo.html](assets/07-api/shadow-demo.html): 演示组件，展示 `this.shadow` 操作和自定义方法定义。
  - [demo.html](assets/07-api/demo.html): 入口文件，演示如何从外部调用组件方法。
