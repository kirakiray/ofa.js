...
组件的 `export default` 函数可接收 `{ load, url }` 参数：
  ...
  <script>
    ...
    export default async ({ load, url }) => {
      ...
      return {
        ...
        data: { htmlContent: "" },
        watch: {
          src(newVal) { if (newVal) this.loadMarkdown(); }
        },
        ready() { if (this.src) this.loadMarkdown(); },
        proto: {
          async loadMarkdown() {
            const response = await fetch(this.src);
            this.htmlContent = await marked.parse(await response.text());
          }
        }
      };
    };
...
**参数说明**：
- `url`：当前模块的完整 URL 地址
- `load`：加载模块/资源，支持 JSON 文件、第三方 ES Module URL，与 `<l-m>` 功能一致且共享缓存
