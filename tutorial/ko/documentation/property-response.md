...
`Green增加` 버튼을 클릭할 때, `_count2`의 값은 실제로 증가했지만 반응형 속성이 아니므로 뷰 업데이트가 트리거되지 않아 인터페이스에 표시되지 않습니다. `Blue增加` 버튼을 클릭하면 `count`는 반응형 속성이므로 전체 컴포넌트의 리렌더링을 트리거하고, 이때 Green의 표시 내용이 동기화되어 업데이트됩니다.
