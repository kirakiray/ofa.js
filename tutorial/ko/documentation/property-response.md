...
`Green increments` 버튼을 클릭할 때는 `_count2`의 값이 실제로는 증가했지만, 이는 비반응형 속성이므로 뷰 갱신을 트리거하지 않아 화면에 표시된 값은 변하지 않는다. `Blue increases` 버튼을 클릭하면 `count`가 반응형 속성이므로 전체 컴포넌트의 재렌더링을 트리거하고, 이때 Green의 표시 내용도 함께 갱신된다.
