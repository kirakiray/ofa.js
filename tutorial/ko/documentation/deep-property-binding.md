...
이전 내용에서는 [속성 바인딩](./property-binding.md)의 기본적인 사용 방법을 간단히 소개했다. 이전 예제는 브라우저 네이티브 요소(예: `textarea`)의 `value` 속성을 바인딩하는 데 사용했으며, 이번 절에서는 속성 바인딩의 본질에 대해 깊이 있게 다룰 것이다. 즉, 속성 바인딩은 실제로 HTML 속성이 아닌 컴포넌트가 인스턴스화된 후의 JavaScript 속성에 바인딩된다는 점이다.
