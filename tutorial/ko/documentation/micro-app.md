...
`o-app`를 사용하여 애플리케이션화합니다. 이 태그는 마이크로 애플리케이션을 나타내며, `app-config.js` 구성 파일을 로드합니다. 이 파일은 애플리케이션의 홈 페이지 주소와 페이지 전환 애니메이션 구성을 정의합니다.
...
지정된 애플리케이션 시작 시 로드되는 홈페이지 모듈 경로로, 상대 경로와 절대 경로를 지원합니다.
...
| 상태 | 설명 |
|------|------|
| `current` | 현재 페이지 애니메이션 종료 후의 스타일 |
| `next` | 새 페이지 진입 시의 시작 스타일 |
...
`o-app`에서 페이지 이동은 URL Query를 통해 파라미터를 전달하는 것을 지원하며, 대상 페이지는 모듈 함수의 `query` 파라미터로 받습니다.

## 페이지 네비게이션

o-app 내에서 각 페이지 모듈은 `olink` 속성이 있는 `<a>` 태그를 사용하여 페이지를 전환할 수 있습니다. 이 태그는 애플리케이션의 라우트 전환을 트리거하며, 전환 애니메이션을 포함하고 전체 페이지를 새로고침하지 않습니다.
