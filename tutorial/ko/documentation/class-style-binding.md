...
스타일에서 `data(key)`를 사용하여 컴포넌트 데이터를 바인딩할 수 있습니다. 이는 컴포넌트 데이터에 따라 스타일을 동적으로 변경해야 하는 시나리오에 매우 적합합니다.
...
<o-playground name="스타일 태그 내의 데이터 함수" style="--editor-height: 500px">
  <code>
    <template page>
      ...
      Hover FontSize: <input type="number" sync:value="size" placeholder="이것은 양방향 바인딩 입력 상자입니다" />
      <br />
      TransitionTime: <input type="number" step="0.3" min="0" sync:value="time" placeholder="이것은 양방향 바인딩 입력 상자입니다" />
      <p>{{val}} - size: {{size}}</p>
      <script>
        export default async () => {
          return {
...
`style` 태그 내의 `data(key)`는 원리적으로 전체 style의 내용을 대체합니다. 불필요한 스타일을 반복 렌더링하지 않도록, `data(key)`를 포함한 스타일은 별도의 `style` 태그에 두고, 데이터 바인딩이 필요 없는 스타일은 다른 `style` 태그에 배치하여 더 나은 성능을 얻는 것을 권장합니다.

```html
<!-- ❌ data(key)가 없는 p:hover도 갱신됩니다 -->
<style>
  p {
    font-size: data(size);
    color:green;
