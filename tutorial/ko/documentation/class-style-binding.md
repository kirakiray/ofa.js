...
<o-playground name="스타일의 데이터" style="--editor-height: 500px">
  <code>
    <template page>
      <style>
        :host {
      ...
      글꼴 크기: <input type="number" sync:value="size" placeholder="양방향 바인딩 입력 상자" />
      <br />
      전환 시간: <input type="number" step="0.3" min="0" sync:value="time" placeholder="양방향 바인딩 입력 상자" />
      <p>{{val}} - size: {{size}}</p>
      <script>
        export default async () => {
          return {
