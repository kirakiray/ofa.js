...
요소 인스턴스는 배열과 유사한 특성을 가지며, 노드를 추가하거나 삭제할 때 배열의那几个 조작 메서드만 사용하면 됩니다. `push`, `unshift`, `pop`, `shift`, `splice` 메서드를 사용할 때는 내부적으로 자동으로 [$ 메서드](../instance/dollar.md)의 초기화 작업이 수행되므로, 구체적인 요소 문자열이나 객체를 바로 입력할 수 있습니다.
