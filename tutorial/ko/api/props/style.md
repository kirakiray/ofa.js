...
请注意，`style` 속성은 스타일의 실제 값을 가져올 수 없으며, 오직 `style` 속성에서 설정된 값만 가져올 수 있습니다. `style` 메서드는 [css 메서드](./css.md)와 유사하지만, 전체 스타일 덮어쓰기는 할 수 없습니다. `css`에 비해, `style` 메서드의 내부 실행 효율이 더 높습니다.

아래는 `style`을 사용하는 예시입니다:
...
기억하세요, `style` 메서드는 `style` 속성의 값만 가져오기와 설정하며, 실제 계산된 스타일이 아닙니다.
