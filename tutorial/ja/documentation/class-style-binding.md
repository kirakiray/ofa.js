...
スタイル内で `data(key)` を使用してコンポーネントデータをバインドできます。これは、コンポーネントデータに基づいてスタイルを動的に変更する必要がある場面に最適です。
...
<o-playground name="スタイルタグ内のデータ関数" style="--editor-height: 500px">
  <code>
    <template page>
      ...
      Hover FontSize: <input type="number" sync:value="size" placeholder="これは双方向バインディングの入力ボックスです" />
      <br />
...
`style` タグ内の `data(key)` は原理的に、スタイル全体を置き換えます。無関係なスタイルが繰り返しレンダリングされるのを避けるため、`data(key)` を含むスタイルは別の `style` タグに分け、データバインディングが不要なスタイルは別の `style` タグに配置して、パフォーマンスを向上させることをお勧めします。

```html
<!-- ❌ data(key) なしの p:hover もリフレッシュされます -->
<style>
  p {
    font-size: data(size);
    color:green;
