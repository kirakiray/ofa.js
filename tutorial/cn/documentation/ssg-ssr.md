...
为了结合 CSR 的良好用户体验和 SSR/SSG 的 SEO 优势，ofa.js 提供了独特的 SCSR（Server Client Side Rendering，服务端客户端渲染）模式。
...
`scsr.mjs` 是 ofa.js 提供的 SCSR 运行引擎，它会根据当前页面的运行状态自动判断渲染策略，确保在任何环境下都能提供最佳的用户体验。