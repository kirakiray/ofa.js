...
`created` 钩子在组件实例创建后立即调用，此时组件的模板尚未渲染，DOM 元素还未生成。这个钩子通常用于初始化组件的状态和设置基本配置。
...
`loaded` 钩子在组件及其所有子组件、异步资源全部加载完毕后触发，此时可安全移除 loading 状态或执行依赖完整组件树的后续操作。
...
1. `created` - 组件实例创建
2. `ready` - 组件准备就绪（DOM 已创建）
3. `attached` - 组件挂载到 DOM
4. `loaded` - 组件完全加载完成
        ...
        export default async () => {
          return {
            ...
            proto: {
              ...
              removeSelf() {
                this.ele.remove(); // 从DOM中移除自身以触发detached钩子
              },
              addLog(message) {
...
生命周期钩子函数是 ofa.js 组件开发中的重要概念，正确使用它们可以帮助你更好地管理组件状态和资源，提升应用性能。
