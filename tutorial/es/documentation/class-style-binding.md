...
<o-playground name="data in style" style="--editor-height: 500px">
  <code>
    <template page>
      ...
      Tamaño de fuente: <input type="number" sync:value="size" placeholder="Este es un cuadro de entrada con enlace bidireccional" />
      <br />
      Tiempo de transición: <input type="number" step="0.3" min="0" sync:value="time" placeholder="Este es un cuadro de entrada con enlace bidireccional" />
      <p>{{val}} - tamaño: {{size}}</p>
      <script>
        export default async () => {
          return {
